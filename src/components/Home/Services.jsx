...
const Services = () => {
  ...
  const renderCard = (service, pop = false) => {
    ...
    return (
      <article
        key={service.id}
        className={`group flex h-full w-[min(68vw,19.5rem)] shrink-0 snap-center flex-col overflow-hidden rounded-2xl border border-white/10 bg-panel sm:w-auto sm:max-w-none sm:shrink sm:snap-align-none sm:rounded-3xl ${
          pop ? 'service-tab' : ''
        }`}
      ...
      >
        ...
        <div className="service-photo relative">
          ...
          {isBts ? (
            <span className="absolute left-2.5 top-2.5 z-[5] rounded-full bg-black/60 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-ivory">
              Reel
            </span>
          ) : null}
          <h3 className="pointer-events-none absolute bottom-2.5 left-3 right-3 z-[5] font-display text-base text-ivory sm:hidden">
            {service.name}
          </h3>
        </div>
        <div className="px-3 py-2.5 sm:px-4 sm:py-3">
          <h3 className="hidden font-display text-lg text-ivory sm:block">{service.name}</h3>
          ...
          {isBridal ? (
            ...
            <button
              type="button"
              onClick={(event) => openGallery(event.currentTarget, 'bts')}
              className="mt-1.5 inline-flex text-[11px] font-semibold text-brand-300 transition hover:text-brand-200 sm:mt-2.5 sm:text-xs"
            >
              Watch reel →
            </button>
  ...
  return (
    <section id="services" className="scroll-target bg-sand">
      <div className="mx-auto w-[min(92%,1360px)] py-9 pb-[max(6.25rem,calc(env(safe-area-inset-bottom)+5.25rem))] sm:py-14 sm:pb-14 lg:py-28">
